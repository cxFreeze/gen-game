...
export class Player extends Character {
    ...
    createPlayer() {
        ...
        merge(this.recalculateAimLine$, PlayerInputs.aimChanged).pipe(throttleTime(25), takeUntil(GameRuntime.disposed$)).subscribe((direction) => {
            this.updateAimLine(direction);
        });

        PlayerInputs.firePressed.pipe(throttleTime(1000 / this.fireRate), takeUntil(GameRuntime.disposed$))
            .subscribe((direction) => {
        ...
        PlayerInputs.firePressed.pipe(debounceTime(50), takeUntil(GameRuntime.disposed$))
            .subscribe(() => {
    ...
    private createAimLine() {
        ...
        const aimMaterial = new StandardMaterial('aimMat', GameRuntime.scene);
