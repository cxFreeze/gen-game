...
import type { BiomeAssetType, EnemyAsset, WorldAsset, ZoneAssetType } from './asset-types';
import { BiomeType, ZoneType } from '../world/world-types';
...
export class AssetManager {
    ...
    private static readonly groundTileSize = Params.chunkSize / 2;
    ...
    static async loadAssets(signal: AbortSignal) {
        ...
        this._projectile = MeshBuilder.CreateSphere('projectile', { diameter: 1 }, GameRuntime.scene);
        this.projectile.isVisible = false;
        this.projectile.isPickable = false;

        // Créer un matériau magique
        const magicMaterial = new StandardMaterial('magicMaterial', GameRuntime.scene);
        ...
        const noiseTexture = new Texture(`${this.texturesPath}/noise.png`, GameRuntime.scene);
        ...
        this._flareSprite = new Texture(`${this.texturesPath}/flare.png`, GameRuntime.scene);
    ...
    private static async loadTownAssets(signal: AbortSignal) {
        ...
        const townGround = new GG3DAsset('townGround', MeshBuilder.CreateDisc('disc', { radius: 250, tessellation: 128 }, GameRuntime.scene), townTextureAsset.material);
        ...
        const plazaGround = new GG3DAsset('plazaGround', MeshBuilder.CreateDisc('disc', { radius: 60, tessellation: 48 }, GameRuntime.scene), plazaTextureAsset.material);
    ...
    private static async load3DAsset(path: string, signal: AbortSignal): Promise<Mesh> {
        signal.throwIfAborted();
        const container = await loadAssetContainerAsync(path, GameRuntime.scene);
        if (signal.aborted) {
            container.dispose();
            signal.throwIfAborted();
        }
    ...
    private static loadTextureAsset(name: string, path: string): TextureAsset {
        const groundMat = new StandardMaterial(name, GameRuntime.scene);
        const texture = new Texture(path, GameRuntime.scene);
